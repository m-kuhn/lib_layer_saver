...
class LayerExporter():
  ...
  def save_layer_definition( self, l ):
    ...
    # Save relations
    QgsProject.instance().relationManager().relations()

    relations = [r for r in QgsProject.instance().relationManager().relations().values() if r.referencingLayer() == l]
    ...
    if not res:
      QMessageBox.warning( iface.mainWindow(), 'Save Named Style', error )
...
class LayerImporter():
  ...
  def load_layer_definition( self, layer ):
    ...
    lt_node = self.read_layer_tree_path( maplayer_node.firstChildElement( 'layer-tree-group' ), QgsProject.instance().layerTreeRoot() )
